...
カード型UIでモードを選択する。Phase 1では市区町村モード、市外局番モードを有効化し、画像判別モードは将来対応として表示のみ行う。
...
問題文に市外局番を表示し、同一市外局番に属する市町村ポリゴンをクリックすれば正解とする。市外局番境界は市町村ポリゴンを基礎に近似し、Turf.js unionで視覚的なまとまりを作る。
...
Phase 2ではデータ構造を共通化し、都道府県やクイズモードを差し替え可能にする。Phase 3以降で関東複数県、全国市区町村、全国市外局番へ広げる。その後、アメリカ州・カナダ市外局番を導入し、アメリカ州旗、カナダ州旗、アメリカ州道標識、アメリカナンバープレートなどの画像判別クイズへ展開する。
