...
- 現行の開始UIで選べる主モードは `市区町村`、`市外局番`、一部データセットの `パズル`
- `確認マップ` は内部モードとして実装されている
- `maBroadFeatures` など広域集約用の事前計算データは一部残っているが、3桁までの広域市外局番専用モードは現方針では採用していない
...
- `maBroadFeatures` — 過去検証用の広域市外局番エリアのフィーチャー
...
> 注記: この節には構想や検証中の項目も含まれます。特に `japan_all` は一度仮実装されましたが、処理が重すぎて実用的ではなかったため、現行の `data/catalog.js` からは外されています。
...
- [ ] 苦手分析ヒートマップの改善
- [ ] GeoGuessr用JSON出力
- [ ] サドンデスを地図なし固定の仕様にそろえる

### 将来
- [ ] 全国版は `都道府県 → 市区町村 → 市外局番` の順で段階的に実装する
- [ ] 画像判別クイズ（旗・標識・ナンバープレート）
- [ ] 画像判別クイズに GeoGuessr 寄りのブラー版を追加する
- [ ] 海外データセット（北米・イギリスなど）
