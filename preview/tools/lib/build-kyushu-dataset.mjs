...
export const KYUSHU_PREFECTURE_CONFIGS = {
  ...
  okinawa: {
    prefectureName: "沖縄県",
    prefectureCode: "47",
    globalName: "OKINAWA_MUNICIPALITIES",
    center: [26.42, 127.89],
    initialView: { center: [26.25, 127.75], zoom: 8 },
    simplify: { interval: "140m", weighting: 0.78, minIslandArea: "14000m2", minSliverArea: "16000m2", sliverControl: 0.94 },
    designatedCities: {},
    areaCodeOverrides: {
      那覇市: "098",
      宜野湾市: "098",
    ...
    maDefaults: { "098": "那覇MA", "0980": "名護MA", "09802": "大東MA" },
    maOverrides: {
      沖縄市: "沖縄MA",
      うるま市: "沖縄MA",
      読谷村: "沖縄MA",
...
function buildMunicipalityMeta(config, areaCodeMaster) {
  return buildEntities(config.prefectureCode, config).map((entity) => {
    ...
    const region =
      config.regionOverrides?.[entity.name] ??
      stripMaSuffix(ma_name);
