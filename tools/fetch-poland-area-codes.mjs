...
const SOURCE_URL = 'https://super-duper.fr/geojson/poland_areacodes2.geojson';
const DATASET_ID = 'poland_area_codes';
const DATASET_GLOBAL = 'POLAND_AREA_CODES_MUNICIPALITIES';
...
async function main() {
  const geojson = await fetchJson(SOURCE_URL, 'Poland area codes GeoJSON');
  ...
  for (const sourceFeature of sorted) {
    ...
    const region = String(properties.name_1 || '').trim() || null;
    const city = String(properties.City || '').trim() || null;
    const id = `poland_area_codes_${code}`;
  ...
  writeWindowAssignment(
    resolve(ROOT, 'data', 'prefectures', 'poland-area-codes.js'),
    DATASET_GLOBAL,
    {
      id: DATASET_ID,
      source: SOURCE_URL,
  ...
  const dataDir = resolve(ROOT, 'data', 'poland-area-codes');
  writeTextFile(
    resolve(dataDir, 'label-overrides.js'),
    'window.POLAND_AREA_CODES_LABEL_OVERRIDES = {\n  municipalities: {},\n  areaCodes: {}\n};\n'
  );
  writeWindowAssignment(
    resolve(dataDir, 'speech-readings.js'),
    'POLAND_AREA_CODES_SPEECH_READINGS',
    speechReadings
  );

  console.log('\n✅ 生成完了: ポーランド Area Code データ');
  console.log('   data/prefectures/poland-area-codes.js');
  console.log('   data/poland-area-codes/label-overrides.js');
  console.log('   data/poland-area-codes/speech-readings.js');
}
